...
<body>
	...
	<div class="row">
		...
		<div class="col col-md-2 offset-md-3" >
			<button type="button" class="btn btn-warning" onclick="PersonasJS.nuevoEmpleado()">
	...
	<%@ include file="modal/modalEmpleado.jsp"%>
	<%@ include file="modal/modalEliminaEmpleado.jsp"%>
  ...
  <script src="${pageContext.request.contextPath}/resources/js/personas/personas.js"></script>
  
  <script type="text/javascript">
  tablaGlobal = $('#tableEmpleados').DataTable({
		columns : [
		// { mData: "id" },
		{
			mData : "nombre"
  ...
  <script>
  	...
  	$("#fechaNac").change(function(){
		var dateParts = $("#fechaNac").val().split("/");
  		var edad = moment().diff(new Date(+dateParts[2], dateParts[1] - 1, +dateParts[0]), 'years');
  		$('#edad').val(edad);
  	}); 
